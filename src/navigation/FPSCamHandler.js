 ...
 * 
 * @param camera
 * @class FPS-inspired control
 * @augments CamHandler
 * @author Markus Schuetz / Nathan Korzekwa
 */
function FPSCamHandler(camera){
	this.camera = camera;
	this.velocity = [0,0,0];
...
FPSCamHandler.prototype = new CamHandler();

FPSCamHandler.prototype.addTime = function(time){
	this.update(time);
};

FPSCamHandler.prototype.update = function(time){
...
FPSCamHandler.prototype.invokeKeyDown = function(event){
	if(event.which === 83 ){
		//s
...
FPSCamHandler.prototype.invokeKeyUp = function(event){
	if(event.which === 83 ){
		//s
...
FPSCamHandler.prototype.invokeKeyPress = function(event){

};

FPSCamHandler.prototype.invokeMouseDown = function(event){
	
};

FPSCamHandler.prototype.invokeMouseUp = function(event){
};

FPSCamHandler.prototype.invokeMouseMove = function(event, diffX, diffY){
	
};

FPSCamHandler.prototype.invokeMouseDrag = function(event, pressedKeys, diffX, diffY){
...
//		this.camera.translate((this.speedMultiplier * -diffX) / 10.0, (this.speedMultiplier * diffY) / 10.0, 0);
		...
		var rotY = M4x4.rotate(-diffX / 100.0, [0, 0, 1], M4x4.I);
		var rotX = M4x4.rotate(-diffY / 100.0, this.camera.getViewVector(), M4x4.I);
...
FPSCamHandler.prototype.invokeMouseWheel = function(delta){
